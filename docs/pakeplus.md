...
-   使用 AI 写出来的一些小工具，例如：课堂随机点名，自己并不懂怎么发布到网上给别人使用，使用 PakePlus 打包成一个 exe 就可以很方便给别人使用了，而且显得我很牛逼 😜
-   高频使用的网站，每天都要先打开一个笨重的浏览器真的很麻烦，使用 PakePlus 打包后的网站，只需要双击一下桌面图标，就可以了，虽然使用桌面快捷方式也可以实现，但是需要打开一个很大的浏览器，就很不美丽了
-   手机打开某个网站的时候，地址栏占据了很大一部分，特别是苹果手机还不能使用万能的插件，那就让 AI 帮我写一个 js 脚本，使用 PakePlus 打包苹果手机 APP 的时候，直接注入 js 脚本，完美解决不能使用浏览器插件的问题
